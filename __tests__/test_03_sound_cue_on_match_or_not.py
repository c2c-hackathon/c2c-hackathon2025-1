...
def test__audio_played_after_matched_buttons(
    solved_game, game_instance, mock_audio_play, mock_button_board
):
    ...
    for i, buttons in enumerate(output_to_buttons.values()):
        ...
        # using >= because on i=7, there should be a "game won" sound played after the correct match
        assert (
            len(mock_audio_play.played_audio) >= (i + 1) * 3
        ), f"No sound played for correct button pair {buttons}"


def test__audio_played_after_non_matched_buttons(
    solved_game, game_instance, mock_audio_play, mock_button_board
):
    output_to_buttons = solved_game
    ...
    for i, buttons in enumerate(all_buttons):
        ...
        assert (
            len(mock_audio_play.played_audio) == (i + 1) * 3
        ), f"No sound played for incorrect button pair {buttons[0]} and {non_matching_button_num}"
