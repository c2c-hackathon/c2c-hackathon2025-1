...
class Game:
    ...
    def _background_logic_checker(self):
        while self.play_game:
            ...
            self.speaker.play_preloaded_wav("bloop_x", wait_until_done=False)  # Play a sound when button is pressed
            self.button_pad.set_button_led_color(button, "red")
            time.sleep(.5) # Keep it lit for half a second
            self.button_pad.set_button_led_color(button, "green")
            time.sleep(.5) # Keep it lit for half a second
            self.button_pad.set_button_led_color(button, "blue")
            time.sleep(.5) # Keep it lit for half a second
